...
configure_defaults() {
	...
	defaults write com.apple.screencapture location -string "${HOME}/tasks/References/Files/Screenshots"
	...
	# TextEdit > Settings > Open and Save > Plain Text File Encoding > UTF-8
	...
	# Activity Monitor > View > Dock Icon > Show CPU Usage
...
if [ $failed -eq 0 ]; then
	...
	err_exit "One or more errors occured during MacOS defaults configuration. Try running _scripts/defaults.sh again."
fi
